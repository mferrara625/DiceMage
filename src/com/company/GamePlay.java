...
public class GamePlay {
    ...
    public static void playDiceMage(Scanner scan, Game game, int input2, boolean isGameActive) {
        Art art = new Art();
        boolean hasAttacked;
        int i;
        int j;
            ...
            if (input.equals("")) {
                for (i = 0; i < 1; i++) {
                    ...
                    while (input2 != 5) {
                        ...
                        else if (input2 == 88888888){
                            System.out.println(game.player.name + " cast SECRET spell...");
                            game.player2.health--;
                        }
                    }
                }
                for (j = 0; j < 1; j++) {
                    ...
                    while (input2 != 5) {
                        ...
                        if (input2 == 5) {
                            ...
                            System.out.println(game.player2.name + " cast SECRET spell...");
                            game.player.health--;
                        }
                    }
                }
    ...
    public static void healCondition(List<Die> dice, Player player) {
        if (dice.size() > 0)
            heal(player, dice);
        else
    ...
    public static void heal(Player player, List<Die> dice) {
    ...
    public static boolean combat(Game game, Player player, Player player2) {
        boolean hasAttacked;
        player.attack();
        player2.attack();
        hasAttacked = true;
    ...
    public static void powerUp(List<Die> dice, Player player) {
    ...
    public static void monstersAttack(int m, Player player, Player player2) {
    ...
    public static void moreMonstersAttack(Player player, Player player2) {
        for (int n = 0; n < player.den.size(); n++) {
            monstersAttack(n, player2, player);
        }
    }

    public static void beginTurn(Game game, Player player2) {
        game.roll();
    ...
    public static void displayField(Game game) {
    ...
    public static boolean addMonster(Scanner scan, Player player) {
        if (player.mana < 13)
    ...
    public static void displayActions(boolean hasAttacked, Player player, Player player2) {
